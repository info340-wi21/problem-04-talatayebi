...
function extractOpponent(gameObject){
  if (gameObject['home'] !== "UW"){
    return gameObject['home'];
  }
  else{
    return gameObject['opponent'];
  }
  }
...
var UWopponents = huskyGames2016['map'](extractOpponent); //https://www.w3schools.com/jsref/jsref_map.asp
console['log'](UWopponents);

//Define a function `huskiesLost()` that takes in a "game" object and returns
//whether or not UW lost

function huskiesLost(gameObject){
  if (gameObject['opponent'] !== "UW" && gameObject['home_score'] > gameObject['opponent_score']){
    return false;
  }
  else if (gameObject['opponent'] == "UW" && gameObject['opponent_score'] > gameObject['home_score']){
    return false;
  }
  else if (gameObject['home'] !== "UW" && gameObject['opponent_score'] > gameObject['home_score']){
  return false;
  } 
  else{
  return true;
...
var gamesUWLost = huskyGames2016['filter'](huskiesLost);
console['log'](gamesUWLost);


//Log out an array of opponents that UW lost to. Hint: Use the `.map()` method 
//to extract the opponent names!

var opponentsUWLostTo = gamesUWLost['map'](extractOpponent);
...
gamesUWLost['forEach'](function(gameObject){
  console['log'](gameObject['opponent'] + 
  ' at ' + gameObject['home'] +
   ', ' + gameObject['opponent_score'] + 
   ' to ' + gameObject['home_score']);
});
...
var fumbles = huskyGames2016['filter'](function(gameObject) {
  if (gameObject['fumbles'] >= 1){
    return true;
  }
  else{
  return false;
...
var numFumbles = fumbles['length'];
console['log'](numFumbles);
...
function mostYardsPassing(firstGame, secondGame){
  ...
  else if (!secondGame['passing_yards']){
    return firstGame;
  }
  else if (!firstGame['passing_yards']){
    return secondGame;
  }
  else {
    return secondGame;
...
function makeCombinedFilter(callbackInput1, callbackInput2) {
  var combinedFilter = function combinedFilter(gameObject) {
    return callbackInput1(gameObject) && callbackInput2(gameObject);
  };
...
var fumbledAndLostFilter = makeCombinedFilter(huskiesLost, function(gameObject){
  var fumbles;
...
var gamesUWLostWithFumbles = huskyGames2016['filter'](fumbledAndLostFilter);
console['log'](gamesUWLostWithFumbles);